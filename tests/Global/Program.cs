...
Check(policy.Allows(dx) && !policy.Allows(vk) && !policy.Allows(mixed) && !policy.Allows(unknown), "Default automatic policy is DX12 only");
...
File.WriteAllText(storage, JsonSerializer.Serialize(new { Applications = new[] { dx, vk, mixed, unknown }.Select(g => new {
...
Check(scan.Games.Count == 4 && scan.Games.Single(g => g.Name == "dx12").Api == GraphicsApi.DirectX12, "Discovery supplies fresh API evidence");
...
Check(installs == 1 && prepares == 1 && saves == 1 && policy.ManagedFolders.Contains(dx.Root), "Only DX12 installs, with ownership persisted first");
...
foreach (uint tier in new uint[] { 1, 26, 0x00FFFFFE, 0x00FFFFFF }) new NvidiaOverridePolicy { FgPreset = tier }.Validate();
Check(true, "All exposed FG preset endpoints validate");
Reject(() => new NvidiaOverridePolicy { FgPreset = 27 }.Validate(), "Reject unknown FG preset");
