...
The 1.2b1 source ZIP downloader and restore-first upgrade flow are retained. Restore deletes the edited INI and runtime logs and restores the original DLL. Runtime upgrades reset the INI to the upstream factory configuration.

## Validation

60 payload, configuration, and recovery checks passed, including the real upstream 0.3.4 source ZIP. Tests cover value mapping, range validation, preservation of comments and unrelated settings, INI-save rollback, journal hashes, original DLL backups, and restoration after editing. Actual game rendering remains game-dependent.
